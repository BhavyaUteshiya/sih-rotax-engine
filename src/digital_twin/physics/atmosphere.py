...
class AtmosphereModel:
    ...
    def calculate(cls, env: EnvironmentInput) -> AtmosphericState:
        ...
        # This is safe for a 30,000 ft (~9,144 m) MALE UAV simulation envelope.
        h = max(0.0, min(env.altitude_m, MAX_ISA_ALTITUDE))
        ...
        # Prevent absolute zero or negative Kelvin for stability
        t_actual_k = max(1.0, t_actual_k)
        ...
        rh = max(0.0, min(env.relative_humidity_pct, 100.0))
        if rh > 0.0:
            # Saturation vapor pressure in hPa
            # Psat = 6.1078 * 10 ^ ((7.5 * Tc) / (Tc + 237.3))
            p_sat_hpa = 6.1078 * math.pow(10, (7.5 * t_actual_c) / (t_actual_c + 237.3))
